...
tc setup codex --plugin --marketplace-path ../sandbox/tc-codex-plugin
tc setup claude --install
...
| 自動配置 | `tc setup claude --install` | `tc setup codex --plugin` | `tc setup gemini` |

## Plugin packages

Claude Code / Codex 向けの plugin パッケージは `plugins/` に配置しています。

```text
plugins/claude-code/  # Claude Code marketplace root
plugins/codex/        # Codex marketplace root
```

各 plugin は `tc mcp` を MCP server として起動し、`.claude/skills/` の skill 群を同梱します。Skill を更新したら、配布前に同期してください。

```bash
bun run sync:plugins
```

ローカル実機検証では、配布物を `sandbox/` にコピーしてから marketplace として install します。

```bash
bun run sync:plugin-sandbox
```

生成先:

```text
../sandbox/tc-claude-plugin  # Claude Code marketplace source
../sandbox/tc-codex-plugin   # Codex marketplace source
```

Claude Code 実機検証:

```bash
tc setup claude --install --marketplace-path ../sandbox/tc-claude-plugin
```

実行後、Claude Code を再起動するか `/reload-plugins` を実行します。
...
このコマンドは従来の MCP + skill 配置に加えて、`~/.codex/config.toml` に local marketplace と `tickercode@tickercode` の有効化設定を追加します。
