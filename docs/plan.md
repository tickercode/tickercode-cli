...
### ✅ Phase 5 — `i_trailing_*` / `i_forward_*` schema rename 追従（2026-04-22 完了）

tickercode-api 側の rename（`../../docs/plan-rename-trailing-forward.md` 参照）が本番 deploy 済み。cli 側も以下で追従:

- [x] `src/memory/mini.ts` の StockItem 型から `i_per` / `i_forecast_*` 系を削除、`i_trailing_*` / `i_forward_*` に差し替え（3 新規フィールドも追加）
- [x] `src/memory/summary.ts` の SUMMARY_OVERVIEW_KEYS 更新（trailing / forward 両方）
- [x] `src/analysis/peers.ts` の pick() / bench マッピング更新（trailing と forward を別フィールドとして両方返す）
- [x] `src/analysis/project.ts` の PER 選択ロジック更新（`miniItem?.i_forward_per` / `miniItem?.i_trailing_per`）
- [x] `src/analysis/moat.ts` 影響確認（`i_trailing_roe` に差し替え）
- [x] MCP tool description を trailing/forward 明示に更新（project_pl）
- [x] `.claude/skills/tc-research/SKILL.md` / `CLAUDE.md` に Agent メンタルモデル（3 ルール + 据え置きフィールド一覧）を明記
- [x] `~/.tickercode/memory/` の既存 cache は次回 fetch で自動上書き（明示 clean は任意）
- [x] dogfooding-log.md に 2026-04-22 エントリを追記
