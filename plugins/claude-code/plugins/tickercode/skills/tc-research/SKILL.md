...
## save_report — 分析結果を web に保存

ユーザーが明示的に「レポートにして」「保存」と依頼した時のみ使用。勝手に使わない。

```
mcp__tickercode__save_report({
  title: "銘柄分析タイトル（60 文字以内）",
  body_markdown: "# 分析\n本文 markdown ...",
  stock_code: "7203",          // 1 銘柄の場合必須
  stock_codes: ["7203","9984"],// 複数銘柄の場合
  one_liner: "一行サマリ",     // 任意
  tags: ["バリュー","製造業"], // 任意
  is_public: false,            // デフォルト false。ユーザーが公開希望時のみ true
})
```

返却: `{ id, slug, url }` — url は https://ticker-code.com/report/{slug}

### 使用ルール

- **ユーザーが明示的に「保存」「レポートにして」と言った時のみ** 呼ぶ
- 分析完了時に自動保存しない
- `is_public` は **ユーザーが「公開」「シェア」と明言した時のみ** true にする。デフォルトは false
- `stock_code` は 4 桁または 5 桁。`normalize_code` で正規化済みなら 5 桁で渡す
- `body_markdown` には会話で生成した分析レポートをそのまま渡す（要約不要）
